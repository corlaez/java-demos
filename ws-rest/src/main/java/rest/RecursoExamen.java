...
public class RecursoExamen {
    ...
    public String examen(@QueryParam("vuelos") String vuelos){
        ...
        //11 puntos
        int maxC = listaVuelos.stream().max(Comparator.comparing(PlanDeVuelo::getCapacidadPasajeros))
                .get().getCapacidadPasajeros();
        var vuelosMaximaCantidad = listaVuelos.stream().filter(v -> v.getCapacidadPasajeros() == maxC)
                .collect(Collectors.toList());
        var vuelosComerciales = listaVuelos.stream().filter(v -> v.getEsComercial())
                .collect(Collectors.toList());
        //2 puntos
